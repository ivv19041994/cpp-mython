...
void TestNumbersSubtraction() {
    runtime::DummyContext context;

    Sub sub(make_unique<NumericConst>(65), make_unique<NumericConst>(34));

    Closure empty;
    ASSERT_OBJECT_VALUE_EQUAL(sub.Execute(empty, context), 31);

    ASSERT(context.output.str().empty());
}

void TestNumbersMultiplication() {
    runtime::DummyContext context;

    Mult mult(make_unique<NumericConst>(65), make_unique<NumericConst>(34));

    Closure empty;
    ASSERT_OBJECT_VALUE_EQUAL(mult.Execute(empty, context), (65 * 34));

    ASSERT(context.output.str().empty());
}

void TestNumbersDivision() {
    runtime::DummyContext context;

    {
        Div div(make_unique<NumericConst>(70), make_unique<NumericConst>(34));
        Closure empty;
        ASSERT_OBJECT_VALUE_EQUAL(div.Execute(empty, context), (70 / 34));
    }
    {
        Div div(make_unique<NumericConst>(68), make_unique<NumericConst>(34));
        Closure empty;
        ASSERT_OBJECT_VALUE_EQUAL(div.Execute(empty, context), (68 / 34));
    }

    {
        Div div(make_unique<NumericConst>(67), make_unique<NumericConst>(34));
        Closure empty;
        ASSERT_OBJECT_VALUE_EQUAL(div.Execute(empty, context), (67 / 34));
    }

    {
        Div div(make_unique<NumericConst>(67), make_unique<NumericConst>(0));
        Closure empty;
        ASSERT_THROWS(div.Execute(empty, context), std::runtime_error);
    }
...
void TestReturn() {
    Closure closure;
    runtime::DummyContext context;
    Return ret{ make_unique <StringConst>(runtime::String("return"))};
    ASSERT(runtime::Equal(ret.Execute(closure, context), ObjectHolder::Own(runtime::String("return")), context));
    ASSERT(context.output.str().empty());
}
...
void RunUnitTests(TestRunner& tr) {
    ...
    RUN_TEST(tr, ast::TestNumbersSubtraction);
    RUN_TEST(tr, ast::TestNumbersMultiplication);
    RUN_TEST(tr, ast::TestNumbersDivision);
